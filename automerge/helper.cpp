...
buffer_blcok::buffer_blcok(usize len) {
...
std::string_view buffer_blcok::cache_string(const std::string& str) {
    auto len = str.size();
    if (len > rest_size()) {
        return {};
    }
...
static buffer_blcok& get_buffer_block(usize len) {
    static std::vector<buffer_blcok> string_buffer;
...
std::string_view cache_string(const std::string& str) {
    ...
    return get_buffer_block(str.size()).cache_string(str);
}
